...
export default function EditarProductoForm({ editarAction, producto }: Props) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4">
      {state.error ? (
        <div className="mb-2 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">
          {state.error}
        </div>
      ) : null}
      ...
      <div className="rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-600">
        ...
        <span className="ml-2 text-gray-400">(modificar solo desde movimientos)</span>
      </div>
      ...
      <label className="flex items-start gap-3 rounded-lg border border-gray-300 px-4 py-3 text-sm text-gray-700">
        ...
        />
        <span>
          <span className="block font-medium text-gray-900">Producto de consumicion</span>
