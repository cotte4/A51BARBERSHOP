...
function formatHeaderDate(fecha: string): string {
...
function addMonthsToDate(fecha: string, months: number): Date {
...
function getDueLabel(date: Date, today: string): string {
...
function getBepProgress(actual: number, target: number): number {
  if (target <= 0) return 0;
  return Math.max(0, Math.min(100, Math.round((actual / target) * 100)));
}

function getBepPips(progress: number): number {
